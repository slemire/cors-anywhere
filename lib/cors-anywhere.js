...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    urlBlacklist: [],               // Request to these URLs will be blocked.
    urlWhitelist: [],               // If non-empty, requests not to an URL in this list will be blocked.
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.urlBlacklist.indexOf(url) >= 0) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The url "' + req.url + '" was blacklisted by the operator of this proxy.');
      return;
    }

    if (corsAnywhere.urlWhitelist.length && corsAnywhere.urlWhitelist.indexOf(req.url) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The url "' + req.url + '" was not whitelisted by the operator of this proxy.');
      return;
    }
